...
impl Plugin for GameWorldPlugin {
    fn build(&self, app: &mut App) {
        app
            ...
            .register_inspectable::<WorldChunk>()
            .register_inspectable::<GameObjectId>()
            .register_inspectable::<GameObjectType>()
...
#[derive(Debug, Clone, Copy, Eq, PartialEq, Hash, Component, Inspectable)]
...
impl WorldChunk {
    ...
    fn asset_load_path(&self) -> HashSet<String> {
...
fn follow_player(player_q: Query<&Transform, With<Player>>, mut events: EventWriter<LoadPosition>) {
        ...
        let entity = match cache.get_object_type(&save, &id) {
            Some(o) => {
                ...
                match o {
                    ...
                    GameObjectType::Player => crate::plugins::player::spawn_player(
                        &mut commands,
                        pos,
                        &mut asset_server,
                        &mut texture_atlases,
...
fn reset_world(
    ...
    for _ in events.iter() {
        ...
        // Unload all object
        let events = transform_query.iter().map(|e| UnloadObject(e));
